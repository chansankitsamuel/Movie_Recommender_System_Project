...
data["overview"] = data["overview"].apply(lambda s:s.split()).apply(str)
def remove_space(s):
    l = []
    for i in str(s):
        l.append(i.replace(" ", ""))
...
data["all"] = data[["genres", "keywords", "overview", "director", "cast"]].apply(lambda x: sum(x.tolist(), []), axis=1).apply(lambda x: ' '.join(x)).apply(lambda x: x.lower())
...
indata.to_csv("processed_data/indata.csv", index=False)